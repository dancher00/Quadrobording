...
def draw_3d_wheel(ax, center, radius, width, resolution=12):
    """Draw a 3D wheel as a cylinder with end caps.
    
    Args:
        ax: Matplotlib 3D axes
        center: (x, y, z) coordinates of wheel center
        radius: Wheel radius
        width: Wheel width
        resolution: Number of points to use for circle approximation
    """
    x, y, z = center
    
    # Create circle points for the two ends of the cylinder
    theta = np.linspace(0, 2 * np.pi, resolution)
    circle_x = radius * np.cos(theta)
    circle_z = radius * np.sin(theta)
    
    # Left and right faces (circles)
    left_y = y - width/2
    right_y = y + width/2
    
    # Draw the cylinder sides
    for i in range(resolution):
        # Connect points on both circles to form the side of the cylinder
        idx1 = i
        idx2 = (i + 1) % resolution
        
        # Points on both ends of the cylinder
        x1, z1 = circle_x[idx1], circle_z[idx1]
        x2, z2 = circle_x[idx2], circle_z[idx2]
        
        # Create a face (quad) connecting points on both circles
        vertices = np.array([
            [x + x1, left_y, z + z1],
            [x + x2, left_y, z + z2],
            [x + x2, right_y, z + z2],
        ...
        # Draw the quad face
        poly = Poly3DCollection([vertices], color='darkgray', alpha=0.1)
        ax.add_collection3d(poly)
    
    # Draw the two circular end caps
    left_circle = np.array([[x + xi, left_y, z + zi] for xi, zi in zip(circle_x, circle_z)])
    right_circle = np.array([[x + xi, right_y, z + zi] for xi, zi in zip(circle_x, circle_z)])
    
    left_cap = Poly3DCollection([left_circle], color='gray', alpha=0.1)
    right_cap = Poly3DCollection([right_circle], color='gray', alpha=0.1)
    
    ax.add_collection3d(left_cap)
    ax.add_collection3d(right_cap)

def draw_skateboard(ax,
                    board_length=0.7,
                    board_width=config_skate_board["width"],
                    board_thickness=0.042,
    ...
    for (xw, yw) in zip(x_wheels, y_wheels):
        wheel_center = (xw, yw, wheel_radius)
        draw_3d_wheel(ax, wheel_center, wheel_radius, wheel_width)
...
def animate_mpc(joint_positions_FR, foot_traj_FR, trunk_dims, trunk_center,
    ...
    lines_FR = [ax.plot([], [], [], color=c, lw=3 if i != 1 else 5)[0] for i, c in enumerate(colors_FR)]
    colors_FL = ['lightcoral', 'red', 'darkred']
    lines_FL = [ax.plot([], [], [], color=c, lw=3 if i != 1 else 5)[0] for i, c in enumerate(colors_FL)]
    colors_RR = ['lightgreen', 'green', 'darkgreen']
    lines_RR = [ax.plot([], [], [], color=c, lw=3 if i != 1 else 5)[0] for i, c in enumerate(colors_RR)]
    colors_RL = ['plum', 'purple', 'indigo']
    lines_RL = [ax.plot([], [], [], color=c, lw=3 if i != 1 else 5)[0] for i, c in enumerate(colors_RL)]
    ...
    for i, c in enumerate(colors_FR):
        h, = ax.plot([], [], [], color=c, lw=3 if i != 1 else 5)
        handles.append(h)
        if i == 0:
    ...
    ax.set_axis_off()  # Отключить оси
    ax.grid(False)     # Отключить сетку
...
def animate_com_transfering(trunk_traj, leg_trajs, com_traj, board_top_z, trunk_dims):
    ...
    lines = {leg: [ax.plot([], [], [], color=c, lw=3 if i != 1 else 5)[0] for i, c in enumerate(colors[leg])] for leg in colors}
